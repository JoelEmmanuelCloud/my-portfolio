        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="max-w-3xl">
            ...
            <p className="max-w-2xl text-lg text-ink/65 dark:text-cream/65 sm:text-xl">
              Paper replications and evaluation experiments on retrieval-augmented generation
              and related AI systems, documented end to end with data and results.
            </p>
          </Reveal>
        </div>
      </section>
          ...
          {research.length > 0 ? (
              ...
              {research.map((piece) => (
                ...
                >
                  <div className="mb-4 flex flex-wrap gap-1.5">
                    {piece.stack?.slice(0, 4).map((tech) => (
                  ...
                  <SlideButton href={`/research/${piece.slug}`} variant="dark">
                    Read the write-up
                  </SlideButton>
                </RevealItem>
              ))}
            ...
            <Reveal className="flex flex-col items-center gap-4 rounded-3xl border border-dashed border-ink/15 bg-white/50 px-6 py-16 text-center dark:border-cream/15 dark:bg-ink/40 sm:py-20">
              ...
              <h3 className="text-xl font-semibold text-ink dark:text-cream sm:text-2xl">
                First write-up in progress
              </h3>
              <p className="max-w-md text-sm leading-relaxed text-ink/60 dark:text-cream/60 sm:text-base">
                Currently working through a retrieval-augmented generation evaluation experiment.
                Check back soon, or get in touch in the meantime.
              </p>
            </Reveal>
          )}
        </div>
