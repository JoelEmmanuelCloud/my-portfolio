                  ...
                  <div className="grid gap-6 sm:gap-8 lg:grid-cols-3 lg:gap-12">
                    <div className="lg:col-span-1">
                      <div className="space-y-3 sm:space-y-4">
                        <h3 className="text-lg sm:text-xl lg:text-2xl font-semibold text-black dark:text-white leading-tight">{job.role}</h3>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm sm:text-base text-black dark:text-gray-300">
                      ...
                      {job.highlights && job.highlights.length > 0 && (
                        <div className="space-y-4">
                          {job.highlights.map((highlight, highlightIndex) => (
                            <p key={highlightIndex} className="text-sm sm:text-base lg:text-lg text-black dark:text-gray-300 leading-relaxed font-medium">
                              {highlight}
                            </p>
                          ))}
                        </div>
                ...
                <div key={index} className="border-2 border-black dark:border-gray-700 rounded-xl p-6 sm:p-8 lg:p-10 bg-white dark:bg-gray-800 hover:shadow-lg transition-shadow duration-300">
                  <div className="flex items-start justify-between mb-4 sm:mb-6">
                    <div className="flex-1 mr-4">
                      ...
                      <p className="text-sm sm:text-base lg:text-lg text-black dark:text-gray-300 font-semibold">{cert.issuer}</p>
                    </div>
                    {cert.verifyUrl && (
                      <a
                        href={cert.verifyUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-600 hover:text-blue-800 dark:hover:text-blue-400 flex-shrink-0 transition-colors duration-200"
                      >
                  ...
                  <div className="space-y-1 sm:space-y-2 text-sm sm:text-base text-black dark:text-gray-300 mb-4 sm:mb-6">
                    <p className="font-medium">{cert.date}</p>
                    {cert.id && <p className="break-all font-medium">ID: {cert.id}</p>}
                  </div>

                  <p className="text-sm sm:text-base lg:text-lg text-black dark:text-gray-300 leading-relaxed font-medium">{cert.description}</p>
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={index} className="border-2 border-black dark:border-gray-700 rounded-xl p-6 sm:p-8 lg:p-10 bg-white dark:bg-gray-800 hover:shadow-lg transition-shadow duration-300">
                  <div className="grid gap-6 sm:gap-8 lg:grid-cols-3 lg:gap-12">
                    <div className="lg:col-span-1">
                      ...
                      <p className="text-sm sm:text-base lg:text-lg text-black dark:text-gray-300 font-semibold mb-4 sm:mb-6">{edu.school}</p>

                      <div className="space-y-3 text-sm sm:text-base text-black dark:text-gray-300">
                        <div className="flex items-center gap-2">
                    ...
                    <div className="lg:col-span-2">
                      <p className="text-sm sm:text-base lg:text-lg text-black dark:text-gray-300 leading-relaxed font-medium">{edu.description}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl text-center lg:text-left mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 justify-center lg:justify-start items-center sm:items-start">
              ...
              <Link
                href="/contact"
                className="w-full sm:w-auto inline-flex items-center justify-center px-6 sm:px-8 lg:px-10 py-3 sm:py-4 lg:py-5 bg-white dark:bg-gray-800 text-blue-600 border-2 border-blue-600 rounded-full hover:bg-blue-50 dark:hover:bg-blue-900/20 active:bg-blue-100 transition-all duration-200 text-sm sm:text-base lg:text-lg font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
              >
                Get In Touch →
              </Link>
            </div>
